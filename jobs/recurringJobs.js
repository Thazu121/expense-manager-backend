  ...
  cron.schedule("0 9 * * *", async () => {
    try {
      ...
      for (const recurring of dueRecurring) {
        const dueDate = recurring.nextDueDate || today;

        const dueKey = `recurring-due-${recurring._id}-${startOfDay(
          dueDate
        ).toISOString()}`;
        ...
        await sendNotification({
          ...
          message: `${recurring.title} is due today - ₹${recurring.amount}`,
          type: "recurring",
          uniqueKey: dueKey,
        });
        ...
        if (!existingExpense && recurring.autoGenerate) {
          await expenseModel.create({
            userId: recurring.userId,
            title: recurring.title,
            amount: Number(recurring.amount || 0),
            category: recurring.category || "General",
            merchant: recurring.merchant || recurring.title,
            ...
            expenseDate: dueDate,
            isRecurring: true,
            recurringType: recurring.frequency,
            recurringExpenseId: recurring._id,
          });

          const generatedKey = `recurring-generated-${recurring._id}-${startOfDay(
            dueDate
          ).toISOString()}`;
          ...
          await sendNotification({
            ...
            message: `${recurring.title} - ₹${recurring.amount} added automatically`,
            type: "recurring",
        ...
        if (recurring.autoGenerate) {
          recurring.lastGeneratedDate = dueDate;
