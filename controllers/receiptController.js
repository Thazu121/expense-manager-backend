...
export const updateReceipt = async (req, res) => {
  try {
    ...
    if (req.body.merchantName !== undefined) {
      updateData.merchantName = req.body.merchantName;
    }

    if (req.body.store !== undefined) {
      updateData.merchantName = req.body.store;
    }

    if (req.body.extractedAmount !== undefined) {
      updateData.extractedAmount = req.body.extractedAmount;
    }

    if (req.body.amount !== undefined) {
      updateData.extractedAmount = req.body.amount;
    }

    if (req.body.extractedDate !== undefined) {
      updateData.extractedDate = req.body.extractedDate;
    }

    if (req.body.date !== undefined) {
      updateData.extractedDate = req.body.date;
    }
    ...
    if (req.body.isVerified !== undefined) {
      updateData.isVerified = Boolean(req.body.isVerified);
    }
    ...
    if (receipt.expenseId) {
      await expenseModel.findByIdAndUpdate(receipt.expenseId, {
    ...
    console.log("UPDATE RECEIPT ERROR:", err);
...
export const getReceiptStatus = async (req, res, next) => {
  try {
    const receipt = await receiptModel.findOne({
      _id: req.params.id,
      userId: req.user.id,
